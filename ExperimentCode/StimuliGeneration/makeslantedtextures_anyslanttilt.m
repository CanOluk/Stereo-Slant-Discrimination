function [ orirandm,phrandm,texL, texR, mean_lum, maskLs, maskRs ] = makeslantedtextures_anyslanttilt(ipd,screenDist,screenWidth,screenWidthPx,slant,tilt,surface_size,shape,insidepatch,surface_sizeinside,aspect_ratio,mean_lum,referenceslant,zrand)
%% Inputs
...
%  location given a specific slant and tilt
...
%MAP rotations to slant and tilt

nORMAL = (Rslant*Rtilt)*[0;0;1];   % normal to the plane
nORMAL = nORMAL'/norm(nORMAL);
CosTheta = dot(nORMAL,[0;0;1])/(norm(nORMAL)*norm([0;0;1]));

    SG = -acosd(CosTheta);

nNew=nORMAL(1:2)./norm(nORMAL(1:2));
CosTheta = dot(nNew,[1;0])/(norm(nNew)*norm([1;0]));
if tilt==0
    TAU = -acosd(CosTheta);
else
TAU = -acosd(CosTheta)*sign(tilt);    %if both minus this must be plus, if just tilt is minus than still minus, if just slant no change
end
if isnan(TAU)
    TAU=0;
end

% decide on the right image, THIS IS WHERE RIGHT IMAGE SHAPE IS DECIDED.
sz_x2 = surface_size_px(1);%
sz_y2 = surface_size_px(2);%
x2 = linspace(-sz_x2,sz_x2,139);
y2 = linspace(-sz_y2,sz_y2,139);

[Xp2,Yp2] = meshgrid(x2,y2);
XRrT=Xp2; 
YRrT=Yp2;
...
ST = sind(TAU)*tand(SG);
CT = cosd(TAU)*tand(SG);
DNOM = ZF - (XRrT-DX).*CT - YRrT.*ST; 
X = (XRrT.*ZP - DX*YRrT.*ST + DX*(ZF-ZP))./DNOM;
YYY = (YRrT.*ZP + DX*YRrT.*CT)./DNOM;
Z = ((XRrT*ZP + DX*(ZF-ZP))*CT + YRrT.*ZP.*ST)./DNOM; %+ ZP;
%figure;imagesc(X-XR);colorbar 
%figure;imagesc(YYY-YR);colorbar
%rotate to get fronto-parallel

someth=(inv(Rslant*  Rtilt))*[X(:)';YYY(:)';Z(:)'];



% get the edges
xS= reshape(someth(1,:),139,139);
yS=reshape(someth(2,:),139,139);
ledgX=[xS(:,1) yS(:,1)];
redgX=[xS(:,end) yS(:,end)];
uedgX=[yS(1,:)' xS(1,:)'];
dedgX=[yS(end,:)' xS(end,:)'];

 % fit some equation to edges potentially lines
 c1 = polyfit(ledgX(:,2),ledgX(:,1),1);
 c2 = polyfit(redgX(:,2),redgX(:,1),1);
 c3 = polyfit(uedgX(:,2),uedgX(:,1),1);
 c4 = polyfit(dedgX(:,2),dedgX(:,1),1);
 
 % masks
 maskL = XL<= Y*c2(1)+c2(2) & XL>= Y*c1(1)+c1(2)...
     & Y<=XL*c4(1)+c4(2)& Y>=XL*c3(1)+c3(2);
 maskR = round(XR,5)<= round(YR*c2(1)+c2(2),5) & round(XR,5)>= round(YR*c1(1)+c1(2),5)...
     & YR<=XR*c4(1)+c4(2)& YR>=XR*c3(1)+c3(2);
 maskLs=maskL;
 maskRs=maskR;
...
% decide on the right image
sz_x2 = surface_sizeinside_px(1);%
sz_y2 = surface_sizeinside_px(2);%
x2 = linspace(-sz_x2,sz_x2,139);
y2 = linspace(-sz_y2,sz_y2,139);
...
% get the edges
xS= reshape(someth(1,:),140,140);
yS=reshape(someth(2,:),140,140);
ledgX=[xS(:,1) yS(:,1)];
redgX=[xS(:,end) yS(:,end)];
uedgX=[yS(1,:)' xS(1,:)'];
dedgX=[yS(end,:)' xS(end,:)'];
 ...
 % masks
 maskLs = XL<= Y*c2(1)+c2(2) & XL>= Y*c1(1)+c1(2)...
     & Y<=XL*c4(1)+c4(2)& Y>XL*c3(1)+c3(2);
 maskRs =  round(XR,5)<= round(YR*c2(1)+c2(2),5) & round(XR,5)>= round(YR*c1(1)+c1(2),5)...
     & YR<=XR*c4(1)+c4(2)& YR>=XR*c3(1)+c3(2);

% The code for the main experiment. To run monocular control experiment, we
% run the same code but specifically choose conditions with 50 degree
% reference slant.
...
sesn=input('enter the session number 1 to 8: \n')
dummy = input(' datasetnumber \n');

% Order of sessions are randomized, either generate the random order or
% read the order.
if exist(strcat(subname,'ss.mat'))==2
    load(strcat(subname,'ss.mat'))
else
    disp('new participant is detected, quit otherwise')
    int=genvarname(subname);
    int=randperm(8);
    save(strcat(subname,'ss.mat'),'int');
end
...
for paofse=1:4
    ...
    % controls reference slant, noise level, feedback, scaling of slant differences,
    % randomseeds for textures, number of repetions for each slant difference
    % level.
    inputs
    %%
    % slant differences between reference and test plane, scale is
    % controlled by the inputs.
    deltaslant=scale.*[-15 -10 -5 -1 1 5 10 15];
    
    % Experiment parameters
    % total number of trial
    numberoftrials=length(slant)*length(deltaslant)*length(noise_levels)*numofrep;
    ...
    zrandomization=round((rand(1,numberoftrials)*2+-1)*10)/10;
    ...
    [orirandm,phrandm,RPtexL, RPtexR, meanlum ,maskLs, maskRs  ] = makeslantedtextures(ipd,screenDist,screenWidth,screenWidthPx,...
        slant,tilt,surface_sizeR,shape(1),insidepatchR,surface_sizeinsideR,aspect_ratioR,mean_lum,refslant,-2);
    ...
    for trial_num = 1:numberoftrials
        ...
        if trial_num==(numberoftrials/2)+1
            ...
            % THIS IS REQUIRED BECAUSE ORIENTATION OF SLANT CHANGES.
        ...
        my_deltaslant=deltaslant(mod(floor(my_trial/length(slant)),length(deltaslant))+1);
        my_noiselevel=noise_levels(mod(floor(my_trial/length(slant)*length(deltaslant)),length(noise_levels))+1);
        my_aspectratio=aspect_ratioE(1,my_trial)';
        my_zrandomization=zrandomization(my_trial);
        ...
        [orirandm,phrandm,ELtexL, ELtexR, ~, maskLsTEST, maskRsTEST ] = makeslantedtextures(ipd,screenDist,screenWidth,screenWidthPx,...
            my_slant+my_deltaslant,tilt,surface_sizeE,shape(2),insidepatchE,surface_sizeinsideE,my_aspectratio,mean_lum,slant,my_zrandomization);
        ...
        if feedback==1
            if (my_deltaslant>0 && observer_response==6) || (my_deltaslant<0 && observer_response==4)
                freqs = 1200;
                duration = 0.2;
                sampleFreq = 44100;
                dt = 1/sampleFreq;
        ...
        fprintf('%d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %d\t %d \n',trial_num, my_trial,my_slant,my_deltaslant, my_noiselevel, my_aspectratio,1,my_zrandomization, reaction_time,observer_response);
        the_output(trial_num,:)=[ trial_num my_trial my_slant my_deltaslant my_noiselevel my_aspectratio 1 my_zrandomization reaction_time observer_response];
        ...
        if trial_num == numberoftrials
            ...
            Screen('FillRect',obj.theWindow,[meanlum])
            Screen('TextSize',obj.theWindow,20)
            Screen('DrawText',obj.theWindow,strcat(num2str(4-paofse),'  parts left in this session'),obj.theRect(3)/2,obj.theRect(4)/2);
    ...
    %% save the data
    % for random seeds, save the first 10 random numbers.
